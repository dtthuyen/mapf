...
for node in nodes:
    ...
    for node2 in nodes:
        ...
        if node[0] == node2[0] and node[1] == node2[1]:
            continue
        if node[0] == node2[0] and (node[1] - node2[1]) < 0 and temp < minYupOfNode:
            minYupOfNode = temp
        if node[0] == node2[0] and (node[1] - node2[1]) > 0 and temp < minYdownOfNode:
            minYdownOfNode = temp
        if node[1] == node2[1] and node[0] - node2[0] < 0 and temp < minXrightOfNode:
            minXrightOfNode = temp
        if node[1] == node2[1] and node[0] - node2[0] > 0 and temp < minXleftOfNode:
            minXleftOfNode = temp
