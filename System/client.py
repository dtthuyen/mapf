...
class Robot:
    ...
    def __init__(
        ...
        self.client.message_callback_add(f"{self.device_id}/path", self.on_message_path)
    ...
    def on_message_path(self, client, obj, msg):
        if msg.payload.decode() == 'path':
            message = json.dumps(self.way)
            client.publish(
                f"{self.device_id}/drawArray", payload=message.encode(), qos=0, retain=False
            )
    ...
    def update(self):
        ...
        if self.run:
            ...
            self.check_drop()
            if len(self.arrMap) == 0 and not self.route_queue:

                if len(self.way):
                    if [self.x, self.y] != [float(self.way[0][0]), float(self.way[0][1])]:
                        self.arrMap = list(self.roamMap)
                    else:
                        self.arrMap = list(self.way[1:])
                else:
                    self.arrMap = list(self.way[1:])
            else:
                if [self.x, self.y] == [float(self.way[0][0]), float(self.way[0][1])] and not self.route_queue:
                    self.arrMap = list(self.way[1:])
